                                ...
                                PtrStep<double> global_buffer)
        {
            ...
            if (x < cols && y < rows) {
                ...
                if (!isnan(normal_current.x())) {
                    ...
                    Eigen::Matrix<float, 3, 1, Eigen::DontAlign> vertex_current_camera =
            ...
            if (correspondence_found) {
                *(Eigen::Matrix<float, 3, 1, Eigen::DontAlign>*) &row[0] = s.cross(n);
                *(Eigen::Matrix<float, 3, 1, Eigen::DontAlign>*) &row[3] = n;
                row[6] = n.dot(d - s);
            } else

...
namespace GPU{
    ...
    namespace CUDA {
        ...
        void update_tsdf_kernel(const PtrStepSz<float> depth_image, const PtrStepSz<uchar3> color_image,
                                ...
                                Eigen::Matrix<float, 3, 3, Eigen::DontAlign> rotation, Eigen::Matrix<float, 3, 1, Eigen::DontAlign> translation)
        {
            ...
            for (int z = 0; z < volume_size.z; ++z) {
                const Eigen::Matrix<float, 3, 1, Eigen::DontAlign> position((static_cast<float>(x) + 0.5f) * voxel_scale,
                                        (static_cast<float>(y) + 0.5f) * voxel_scale,
                                        (static_cast<float>(z) + 0.5f) * voxel_scale);
                const Eigen::Matrix<float, 3, 1, Eigen::DontAlign> camera_pos = rotation * position + translation;

                if (camera_pos.z() <= 0){
                    continue;
                }

                const Eigen::Matrix<int, 2, 1, Eigen::DontAlign> uv(
                ...
                const Eigen::Matrix<float, 3, 1, Eigen::DontAlign> xylambda(

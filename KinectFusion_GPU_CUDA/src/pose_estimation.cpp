...
namespace GPU{
    namespace CUDA { // Forward declare CUDA functions
        void estimate_step(const Eigen::Matrix3f& rotation_current, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& translation_current,
                            const cv::cuda::GpuMat& vertex_map_current, const cv::cuda::GpuMat& normal_map_current,
                            const Eigen::Matrix3f& rotation_previous_inv, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& translation_previous,
...
bool pose_estimation(Eigen::Matrix4f& pose,
    ...
    for (int level = pyramid_height - 1; level >= 0; --level) {
        for (int iteration = 0; iteration < iterations[level]; ++iteration) {
            ...
            // cv::imshow("ICP Frame vertex", frame_data.vertex_pyramid[level]);
            // cv::imshow("ICP Frame normal", frame_data.normal_pyramid[level]);
            // cv::imshow("ICP Model vertex", model_data.vertex_pyramid[level]);
            ...
            // std::cout << "camera_translation_incremental: \n" << camera_translation_incremental << std::endl;
            // std::cout << "camera_rotation_incremental: \n" << camera_rotation_incremental << std::endl;
            ...
            if (!isnan(normal_current.x())) {
                ...
                if (point.x() >= 0 && point.y() >= 0 && point.x() < cols && point.y() < rows && vertex_current_camera.z() >= 0){
                    ...
                    if (!isnan(normal_previous_global.x())){
                        ...
                        if (distance <= distance_threshold){
                            // if (true){
                            //     std::cout << "vertex_previous_global(" << x << ", " << y << "): \n" << vertex_previous_global << std::endl;
                            //     std::cout << "vertex_current_global(" << x << ", " << y << "): \n" << vertex_current_global << std::endl;
                            //     std::cout << "distance: " << distance << std::endl;
                            // }
                            normal_current.y() = normal_map_current.at<cv::Vec3f>(y, x)[1];
                            normal_current.z() = normal_map_current.at<cv::Vec3f>(y, x)[2];

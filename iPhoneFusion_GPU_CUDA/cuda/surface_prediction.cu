...
namespace GPU{
    namespace CUDA{
        ...
        float interpolate_trilinearly(const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& point, const PtrStepSz<short2>& volume,
        ...
        __device__ __forceinline__
        float get_min_time(const float3& volume_max, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& origin, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& direction)
        {
        ...
        __device__ __forceinline__
        float get_max_time(const float3& volume_max, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& origin, const Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& direction)
        {
                                    ...
                                    const Eigen::Matrix<float, 3, 1, Eigen::DontAlign> translation)
        {
            ...
            for (; ray_length < max_search_length; ray_length += truncation_distance * 0.5f) {
                ...
                if (previous_tsdf > 0.f && tsdf < 0.f) { //Zero crossing
                    ...
                    Eigen::Matrix<float, 3, 1, Eigen::DontAlign> normal, shifted;
                            ...
                            const Eigen::Matrix4f& pose)
    {
        ...
        CUDA::raycast_tsdf_kernel<<<blocks, threads>>>(volume.tsdf_volume, volume.color_volume,
